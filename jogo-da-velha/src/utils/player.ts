...
import { isTerminal, getAvailableMoves, printFormateBoard } from "./board";
...
number => {
    ...
    const getBestMoveRecursive = (state: BoardState, maximizing: 
        boolean, depth = 0, maxDepth = -1): 
    number => {
        ...
        if(maximizing){
            ...
            getAvailableMoves(state).forEach(index => {
                ...
                if(depth === 0){
                    ...
                    `${childValues[childValue]},${index}` : `${index}`;
                }
            })
            if( depth === 0) {
